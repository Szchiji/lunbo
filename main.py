...
from modules.scheduler import (
    ...
    add_text, add_media, add_button, add_repeat, add_period,
    ...
    toggle_status, toggle_remove_last, toggle_pin,
    delete_schedule_callback,
    SELECT_GROUP, ADD_TEXT, ADD_MEDIA, ADD_BUTTON,
    ADD_REPEAT, ADD_PERIOD, ADD_START_DATE, ADD_END_DATE,
    ADD_CONFIRM, EDIT_TEXT, EDIT_MEDIA, EDIT_BUTTON,
    EDIT_REPEAT, EDIT_PERIOD, EDIT_START_DATE, EDIT_END_DATE
)
from scheduled_sender import scheduled_sender
from modules.keyboards import (
    schedule_list_menu, group_feature_menu, group_select_menu
)
from modules.keywords_reply import (
    KW_ADD, KW_EDIT,
    keywords_setting_entry, kw_add_start, kw_add_receive,
    kw_remove, kw_remove_confirm, kw_enable, kw_enable_confirm,
    kw_disable, kw_disable_confirm, kw_delay, kw_delayset_confirm,
    keyword_autoreply, kw_edit, kw_edit_entry, kw_edit_save
)
...
async def schedule_entry(update, context):
    await update.message.reply_text(
        "请选择群聊：", reply_markup=group_select_menu(GROUPS)
    )
...
async def back_to_prev_callback(update, context):
    ...
    await update.callback_query.edit_message_text(
        f"已选择群聊：{group_name}\n请选择要管理的功能：\n\n"
        f"操作时间：{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        reply_markup=group_feature_menu(group_id, group_name=group_name)
    )
...
def main():
        ...
        states={
            ...
            ADD_MEDIA: [
                MessageHandler(
                    ...
                    (~filters.COMMAND), add_media
                )
            ],
            ADD_BUTTON: [
                MessageHandler(filters.TEXT & (~filters.COMMAND), add_button)
            ],
            ADD_REPEAT: [
                MessageHandler(filters.TEXT & (~filters.COMMAND), add_repeat)
            ],
            ADD_PERIOD: [
                MessageHandler(filters.TEXT & (~filters.COMMAND), add_period)
            ],
            ADD_START_DATE: [
                MessageHandler(filters.TEXT & (~filters.COMMAND), add_start_date)
            ],
            ADD_END_DATE: [
                MessageHandler(filters.TEXT & (~filters.COMMAND), add_end_date)
            ],
            ADD_CONFIRM: [
            ...
            EDIT_MEDIA: [
                MessageHandler(
                    (filters.PHOTO | filters.VIDEO | filters.Document.ALL | filters.TEXT) &
                    (~filters.COMMAND), edit_media_save
                )
            ],
            EDIT_BUTTON: [
    ...
    keyword_conv = ConversationHandler(
        ...
        states={
            KW_ADD: [
                MessageHandler(filters.TEXT & (~filters.COMMAND), kw_add_receive)
            ],
            KW_EDIT: [
                MessageHandler(filters.TEXT & (~filters.COMMAND), kw_edit_save)
            ],
        },
        fallbacks=[
    ...
    application.add_handler(
        MessageHandler(filters.TEXT & filters.ChatType.GROUPS, keyword_autoreply)
    )
    ...
    async def on_startup(app):
        ...
        app.bot_data["bg_task"] = asyncio.create_task(
